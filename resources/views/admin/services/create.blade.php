...
@section('title', 'Tambah Layanan - NARIS STEAM')
...
<div class="flex items-center gap-2 text-sm text-gray-500">
    ...
    <span class="text-gray-800">Tambah Layanan</span>
</div>
@endsection
...
<div class="max-w-2xl">
    <div class="rounded-2xl bg-white p-6 shadow-sm border border-gray-100">
        <div class="mb-6">
            ...
            <p class="text-sm text-gray-500">Isi form di bawah untuk menambah layanan cuci</p>
        </div>
        ...
        <form action="{{ route('admin.services.store') }}" method="POST" class="space-y-5">
            ...
            <div>
                ...
                <input type="text" 
                       ...
                       placeholder="Contoh: Steam Biasa - Motor Kecil"
                       required>
                @error('name')
            ...
            <div>
                ...
                <select id="category" 
                        name="category" 
                        class="w-full rounded-xl border border-gray-200 px-4 py-2.5 text-sm focus:border-blue-500 focus:outline-none focus:ring-2 focus:ring-blue-500/20 @error('category') border-red-500 @enderror"
                        required>
                    <option value="">Pilih Kategori</option>
                    <option value="steam" {{ old('category') == 'steam' ? 'selected' : '' }}>Steam Biasa</option>
                    <option value="premium" {{ old('category') == 'premium' ? 'selected' : '' }}>Premium Wash</option>
                </select>
                @error('category')
                    <p class="mt-1 text-sm text-red-500">{{ $message }}</p>
                @enderror
            ...
            <div>
                ...
                <select id="vehicle_type" 
                        name="vehicle_type" 
                        class="w-full rounded-xl border border-gray-200 px-4 py-2.5 text-sm focus:border-blue-500 focus:outline-none focus:ring-2 focus:ring-blue-500/20 @error('vehicle_type') border-red-500 @enderror"
                        required>
